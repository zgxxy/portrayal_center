...
import { getActiveLocalProjectByPage } from '@/api/sceneApplication'
import { downloadReportFile, downloadContrastReportFile } from '@/api/report'
import { downloadFile } from '@/utils/file'
import { thousandFormat } from '@/utils/public'
import heatmapData from '/public/testData/heatmapData.json'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      getActiveLocalProjectByPage(params).then((res) => {
    ...
    gotoDetail(row, event) {
      ...
      if (event.label !== '操作' && row.isGeneral == 1) {
        this.$router.push({
          path: '/sceneApplication/addressInfoReport',
          query: row,
        })
      }
    },
    ...
    openDialog() {
      this.$router.push({
        path: '/sceneApplication/eventSite',
        // query: row,
      })
